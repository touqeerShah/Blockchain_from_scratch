...
class PubSub {
  ...
  async connectPubSub() {
    ...
    await subscriber.subscribe(CHANNEL.TEST, (message) => {
      console.log("message", message); // 'message'
    });
    //           console.log("here4");
    // this.subscriber.on(
...
async function test() {
  ...
  console.log("here5");
  await testPubSub.publisher.publish(
    "article",
    JSON.stringify({ article: "etst" })
  );
  console.log("here6");
  await testPubSub.publisher.publish(CHANNEL.TEST, "foo");
