...
describe("Transaction", async function () {
  ...
  describe("Output Map", async function () {
    ...
    it("check amount to the sender", function (done) {
      assert.equal(
        transaction.outputMap[senderWaller.publicKey],
        senderWaller.balance - amount
      );
  ...
  describe("update ", async function () {
    ...
    describe("update with some receiver ", async function () {
      ...
      it("update existing sender amount", function (done) {
        ...
        assert.equal(
          transaction.outputMap[senderWaller.publicKey],
          originalSenderOutput - nextAmount - addnewAmount
        );
  ...
  describe("Reward Transaction ", async function () {
    let rewardTransaction, minerWallet;
    before(async () => {
      minerWallet = new Wallet();
      rewardTransaction = Transaction.rewardTransaction({ minerWallet });
    });
    it("creates a transaction with the reward input", function (done) {
      // assert.deepProperty(wallet, "balance")
      expect(rewardTransaction.input).to.deep.equal(REWARD_INPUT);
      done();
    });
    it("creates ones transaction for thr miner thr MINING_REWARD", function (done) {
      assert.equal(
        rewardTransaction.outputMap[minerWallet.publicKey],
        MINER_REWARD
      );
